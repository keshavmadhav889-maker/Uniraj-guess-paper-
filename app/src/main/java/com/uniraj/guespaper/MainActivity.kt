...
fun UnirajApp() {
 ...
 MaterialTheme {
  if (token == null) AuthScreen { t,u -> token=t; user=u }
  else when(screen) {
...
private fun AuthScreen(onAuth:(String,User)->Unit) {
 var register by remember { mutableStateOf(false) }; var name by remember { mutableStateOf("") }; var email by remember { mutableStateOf("") }; var password by remember { mutableStateOf("") }
 var error by remember { mutableStateOf<String?>(null) }; var busy by remember { mutableStateOf(false) }; val scope=rememberCoroutineScope()
 Column(Modifier.fillMaxSize().padding(24.dp),verticalArrangement=Arrangement.Center) {
  Text("Uniraj Guess Paper",style=MaterialTheme.typography.headlineMedium); Text("Rajasthan University • Guess Papers")
  Spacer(Modifier.height(24.dp)); if(register) OutlinedTextField(name,{name=it},label={Text("Name")},modifier=Modifier.fillMaxWidth())
...
private fun HomeScreen(token:String,user:User,onLogout:()->Unit,onPurchases:()->Unit,onOrders:()->Unit) {
 var papers by remember { mutableStateOf<List<Paper>>(emptyList()) }; var error by remember { mutableStateOf<String?>(null) }; val scope=rememberCoroutineScope(); val context=LocalContext.current
 ...
 Scaffold(topBar={TopAppBar(title={Text("Uniraj Guess Paper")},actions={TextButton(onClick=onPurchases){Text("My Papers")};TextButton(onClick=onOrders){Text("Orders")};TextButton(onClick=onLogout){Text("Logout")}})}){pad->
  LazyColumn(Modifier.padding(pad).padding(16.dp),verticalArrangement=Arrangement.spacedBy(12.dp)){item{Text("Welcome, ${user.name}",style=MaterialTheme.typography.titleLarge);Text("Choose your Rajasthan University guess paper")};error?.let{item{Text(it,color=MaterialTheme.colorScheme.error)}};items(papers){paper->PaperCard(paper,token,context,scope)}}
 }
}

@Composable
private fun PaperCard(paper:Paper,token:String,context:android.content.Context,scope:CoroutineScope){var show by remember{mutableStateOf(false)};Card(Modifier.fillMaxWidth()){Column(Modifier.padding(16.dp)){Text(paper.title,style=MaterialTheme.typography.titleMedium);Text("${paper.subject} • ${paper.semester?:""}");Text("₹${paper.price_paise/100}",style=MaterialTheme.typography.titleLarge);Spacer(Modifier.height(8.dp));Button(onClick={show=true}){Text("Buy with PhonePe / UPI")}}};if(show)PaymentDialog(paper,token,context,scope){show=false}}

@Composable
private fun PaymentDialog(paper:Paper,token:String,context:android.content.Context,scope:CoroutineScope,onDismiss:()->Unit){
 ...
 AlertDialog(onDismissRequest=onDismiss,title={Text("Pay ₹${paper.price_paise/100}")},text={Column{Text("UPI ID: ${order?.upiId?:"7300349017@ybl"}");Text("Merchant: Nitin Saini");Spacer(Modifier.height(8.dp));Text("1. Tap Open PhonePe / UPI.\n2. Pay the exact amount.\n3. Enter your UPI transaction/reference ID below.");Spacer(Modifier.height(8.dp));OutlinedTextField(txn,{txn=it},label={Text("UPI Transaction ID")},modifier=Modifier.fillMaxWidth());msg?.let{Text(it,color=MaterialTheme.colorScheme.error)}}},confirmButton={Button(enabled=order!=null&&!busy&&txn.isNotBlank(),onClick={scope.launch{busy=true;try{val r=ApiProvider.api.submitPayment(token,order!!.orderRef,SubmitPaymentRequest(txn));msg=r["message"]?.toString()?:"Submitted"}catch(e:Exception){msg=e.message?:"Submit failed"};busy=false}}){Text(if(busy)"Submitting…" else "Submit Payment")}},dismissButton={TextButton(onClick={order?.let{launchUpi(context,it.upiId,paper.price_paise/100)}}){Text("Open PhonePe")}})
}
...
@Composable
private fun PurchasesScreen(token:String,onBack:()->Unit){val context=LocalContext.current;var purchases by remember{mutableStateOf<List<Purchase>>(emptyList())};LaunchedEffect(Unit){try{purchases=ApiProvider.api.purchases(token)}catch{}};Column(Modifier.fillMaxSize().padding(16.dp)){TextButton(onClick=onBack){Text("← Back")};Text("My Papers",style=MaterialTheme.typography.headlineSmall);Spacer(Modifier.height(12.dp));LazyColumn(verticalArrangement=Arrangement.spacedBy(8.dp)){items(purchases){p->Card(Modifier.fillMaxWidth()){Column(Modifier.padding(12.dp)){Text(p.title);Text("${p.subject} • Purchased");p.pdfUrl?.let{url->Button(onClick={try{context.startActivity(Intent(Intent.ACTION_VIEW,Uri.parse(url)))}catch{}}){Text("Open PDF")}}}}}}}}
